...
from app.models import Project, OnlineUser, UserSession
...
def create_project(project: schemas.ProjectCreate, db: Session = Depends(get_db)):
    ...
    print("Saving project: {name}, {description}")
...
def admin_page(
    ...
    return templates.TemplateResponse(
        "adminPanel.html", {"request": request, "projects": projects}
    )
...
class ConnectionManager:
    ...
    async def disconnect(self, session_id: str):
        if session_id in self.active_connections:
            ...
            user = (
                self.db.query(OnlineUser)
                .filter(OnlineUser.session_id == session_id)
                .first()
            )
            if user:
    ...
    async def cleanup_inactive_users(self):
        ...
        self.db.query(OnlineUser).filter(OnlineUser.last_seen < one_hour_ago).delete()
        self.db.commit()
